...
void Level::OnRender() {
	for (auto& el : m_Floors) {
		VilagOS::Renderer2D::DrawQuad(el.position, el.size, el.m_GrassTexture);
	}
	...
	VilagOS::Renderer2D::DrawQuad(glm::vec3{ -8.0f, 8.0f, -0.9f }, glm::vec2{-40.0f, -40.0f}, m_BackgroundTexture);
	m_Player.OnRender();
}
...
void Level::CreateFloors() {
	m_Floors.resize(9);
	for (int i = 0; i < 9; i++) {
		m_Floors[i].m_GrassTexture.reset(new VilagOS::Texture2D("assets/textures/grass.png"));
		m_Floors[i].position = glm::vec3(-14.1f + 5 * i, -8.0f, -0.1f);
		m_Floors[i].size = glm::vec2(-5.1f, -5.0f);
	}
}

void Level::CreatePlatforms() {
	m_FlyingStuff.resize(2);
	m_FlyingStuff[0].m_GrassTexture.reset(new VilagOS::Texture2D("assets/textures/grass.png"));
	m_FlyingStuff[0].position = glm::vec3(3.0f, -1.0f, -0.3f);
	m_FlyingStuff[0].size = glm::vec2(-5.1f, -0.9f);

	m_FlyingStuff[1].m_GrassTexture.reset(new VilagOS::Texture2D("assets/textures/grass.png"));
...
bool CollidedWithFlyingThing(glm::vec2 vert, glm::vec2 vertOne, glm::vec2 vertTwo, glm::vec2 vertThree, glm::vec2 vertFour) {
	float DOne = (vertTwo.x - vertOne.x) * (vert.y - vertOne.y) - (vert.x - vertOne.x) * (vertTwo.y - vertOne.y);
	float DTwo = (vertThree.x - vertTwo.x) * (vert.y - vertTwo.y) - (vert.x - vertTwo.x) * (vertThree.y - vertTwo.y);
	float DThree = (vertFour.x - vertThree.x) * (vert.y - vertThree.y) - (vert.x - vertThree.x) * (vertFour.y - vertThree.y);
	float DFour = (vertOne.x - vertFour.x) * (vert.y - vertFour.y) - (vert.x - vertFour.x) * (vertOne.y - vertFour.y);
...
bool Level::OnPickup() {
	...
	for (auto& script : m_Scripts) {
		...
		for (int j = 0; j < 4; j++) {
			if (CollidedWithFlyingThing(glm::vec2(playerTransformedVerticies[j].x, playerTransformedVerticies[j].y), floorVerticies[0], floorVerticies[1], floorVerticies[2], floorVerticies[3])) {
