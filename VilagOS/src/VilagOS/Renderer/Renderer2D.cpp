...
namespace VilagOS {
	...
	void Renderer2D::Init() {
		...
		s_Data.TextureShader.reset(new Shader("assets/shaders/FlatColor.glsl"));
	...
	void Renderer2D::DrawQuad(const glm::vec3& position, glm::vec2 size, const glm::vec4& color) {
		s_Data.TextureShader->UploadUniformVec4(color, "u_Color");
		s_Data.TextureShader->UploadUniformFloat(1.0f, "u_Tiling");
	...
	void Renderer2D::DrawQuad(const glm::vec3& position, glm::vec2 size, const std::shared_ptr<Texture2D>& texture, float tiling){
		//s_Data.TextureShader->Bind();
		s_Data.TextureShader->UploadUniformVec4(glm::vec4(1.0f), "u_Color");
		s_Data.TextureShader->UploadUniformFloat(tiling, "u_Tiling");
		texture->Bind();
